...
Simple analizador JSON escrito en Java.

<br>

### Uso Básico
Analizar un JSON es muy sencillo, solo necesitas llamar al método `parse` y listo.
...
Puedes imprimir el objeto de forma legible usando:
...
Para acceder a sus valores usa:
...
Analizar un array es igual de sencillo que un objeto.
...
### Analizando valores personalizados
Primero necesitamos crear nuestro tipo de dato:

```java
record Item(String name, int quantity, double price) { }
```

Entonces necesitamos un `JsonConverter` para convertir nuestro tipo de dato a JSON y viceversa.
...
Recuerda registrar tu convertidor para que la librería pueda usarlo.
...
Listo. Ahora estamos listos para trabajar con él.
...
### Validación de Datos
De igual forma podemos validar nuestros datos antes de procesarlos. Tomemos el convertidor anterior como ejemplo.
...
class ItemJsonConverter implements JsonConverter<Item> {
    ...
    // Recomiendo reutilizar el validador
    ...
    @Override
    public Item convert(JsonObject value) {
        validator.validate(value); // Aplica la validación antes de procesar los datos.
        return new Item(
                value.object().get("name").string(),
                value.object().get("quantity").integral(),
                value.object().get("price").decimal()
    ...
    public JsonValue convert(Item value) {
        ...
        object.put("quantity", new JsonIntegral(value.quantity()));
        object.put("price", new JsonDecimal(value.price()));
        // Tambien puedes validar los datos despues de procesarlos
...
Actualmente esta librería soporta los siguientes tipos de datos, todos ellos implementan la interfaz `io.github.cuisse.json.JsonValue`.
...
### Rendimiento
El rendimiento es bastante bueno, he ejecutado una prueba de rendimiento contra otros
analizadores JSON populares en Java y los resultados son decentes. Puedes echar un vistazo abajo.
...
<details>
  <summary>Registro Detallado</summary>
...
Por supuesto, siempre puedes ejecutar tus propias pruebas de rendimiento. Los datos JSON utilizados en esta prueba son los siguientes.
...
### Contribuciones
Un poco de ayuda siempre es apreciada, si sientes que algo puede ser mejorado por favor házmelo saber mediante un pull request. Gracias de antemano.
